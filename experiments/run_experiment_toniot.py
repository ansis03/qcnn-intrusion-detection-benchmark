"""
ToN_IoT(Network 서브셋) 실험 — run_experiment.py/run_experiment_cicids.py와 동일 프로토콜.

데이터는 data/raw/toniot/train_test_network.csv 에 두고 사용(README 다운로드 안내 참고).
...
from preprocessing_toniot import load_and_preprocess
...
CSV_PATH = os.path.join(BASE, 'data', 'raw', 'toniot', 'train_test_network.csv')
...
def run(n_sizes, n_repeats, steps, ansatz=ANSATZ, u_params=U_PARAMS, results_dir_name=RESULTS_DIR_NAME):
    ...
    print("=== ToN_IoT(Network) 로드 ===")
    X_train, X_test, y_train, y_test = load_and_preprocess(CSV_PATH, n_components=N_QUBITS)
