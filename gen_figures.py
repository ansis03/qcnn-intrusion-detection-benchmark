...
ax.set_title('QCNN Underperforms Classical Baselines on NSL-KDD')
...
ax.set_title('A More Expressive Ansatz Narrows the Generalization Gap')
ax.legend(loc='upper right')
ax.grid(True, which='major', linestyle='--', alpha=0.4)
plt.tight_layout()
plt.savefig(os.path.join(FIGURES_DIR, 'fig2_gen_gap.png'), dpi=300, bbox_inches='tight')
plt.close()
print('Saved: fig2_gen_gap.png')
...
fig.suptitle('A More Expressive Ansatz Is Both More Accurate and More Stable (NSL-KDD)', y=1.04)
plt.tight_layout()
...
fig.suptitle('QCNN vs. Classical Baselines: the Gap Size Depends on the Dataset (N ≤ 5,000)', y=1.03)
plt.tight_layout()
...
fig.suptitle('Does More Training Data Close the Gap? Only on Some Datasets', y=1.04)
plt.tight_layout()
